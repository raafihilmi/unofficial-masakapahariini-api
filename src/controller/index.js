...
const fetchRecipes = (req, res, response) => {
  try {
    ...
    element.find('._recipe-card .card').each((i, e) => {
      ...
      duration = $(e)
        .find('._recipe-features a')
        .text()
        .trim()
        .split('\n')[0]
        .trim();
      difficulty = $(e)
        .find('._recipe-features a[data-tracking]')
        .last()
        .text()
        .replace('\n', '')
        .trim();
...
const fetchCategory = (req, res, response, index) => {
  ...
  $(element.find('ul.sub-menu')[index]).each((i, e) => {
    // loop untuk link
    $(e)
      .find('li a')
      .each((index, el) => {
...
const fetchProduct = (req, res, response, index) => {
  ...
  $(element.find('.mb-5.mb-md-7 ul')[index]).each((i, e) => {
    // loop untuk link
    $(e)
      .find('li a')
      .each((index, el) => {
...
const Controller = {
  ...
  recipesDetail: async (req, res) => {
    try {
      ...
      duration = elementHeader
        .find('._recipe-features')
        .find('a:not([data-tracking])')
        .text()
        .trim();
      difficulty = elementHeader
        .find('._recipe-features a[data-tracking]')
        .last()
        .text()
        .trim();
      calories = elementHeader
        .find('._recipe-features a.icon_fire')
        .last()
        .text()
        .trim();
